...
const Index = () => {
  ...
  useEffect(() => {
    ...
    const fetchFeatured = async () => {
      ...
      try {
        ...
        let categories = catRes?.data?.categories || catRes?.categories || catRes?.data || [];
  ...
  useEffect(() => {
    ...
    const fetchNewLaunches = async () => {
      ...
      try {
        const res = await userAxios.get("/products", { params: { sort: 'createdAt', order: 'desc', limit: 6 } });
  ...
  useEffect(() => {
    ...
    const fetchHandPicked = async () => {
      ...
      try {
        ...
        const res = await userAxios.get("/products", { params: { sort: 'variantDetails.colour', order: 'asc', limit: 4 } });
  ...
  const handleToggleWishlist = (product, e) => {
    ...
    const isWishlisted = wishlist.some(item => item.id === productId && item.variant === variantId);
    if (isWishlisted) {
    ...
    <div>
      ...
      <Container className="py-5">
        ...
        {loadingFeatured ? (
          ...
          <Row xs={1} md={2} lg={4} className="g-4">
            {featuredProducts.length > 0 && featuredProducts.some(fp => fp.product) ? (
              featuredProducts.map((fp, i) => {
                ...
                const variant = (product.variants && product.variants.length > 0 && product.variants[0]) ||
                                (product.variantDetails && product.variantDetails.length > 0 && product.variantDetails[0]) ||
                                null;
                ...
                const price =
                  ...
                  (product.variantDetails && product.variantDetails.length > 0 && product.variantDetails[0].price) ||
                  product.price ||
                  "--";
                // Wishlist logic
                const isWishlisted = (() => {
                  ...
                  const variantId = variant?._id || variant?.id;
                  const productId = product._id || product.id;
                  return variantId ? wishlist.some(item => item.id === productId && item.variant === variantId) : false;
                })();
                return (
                  <Col key={product._id || i}>
                    <Card className="h-100 shadow-sm hover-shadow" onClick={() => navigate(`/products/${product._id}`)} style={{ cursor: "pointer" }}>
                      <div className="position-relative">
                        ...
                        <Button
                          ...
                          style={{ top: '8px', right: '8px', width: '30px', height: '30px', padding: 0, border: 'none', backgroundColor: 'rgba(255, 255, 255, 0.95)', boxShadow: '0 2px 8px rgba(0,0,0,0.15)', zIndex: 10, transition: 'all 0.2s ease' }}
                          onClick={(e) => { e.stopPropagation(); handleToggleWishlist(product, e); }}
                          aria-label={isWishlisted ? 'Remove from wishlist' : 'Add to wishlist'}
                        >
                          <FaHeart
                            className={isWishlisted ? "text-danger" : ""}
                            style={{ color: isWishlisted ? undefined : "#555", filter: !isWishlisted ? "drop-shadow(0 1px 2px rgba(0,0,0,0.10))" : "none" }}
                            size={20}
                      ...
                      <Card.Body>
                        ...
                        <Card.Text className="text-muted small">{category?.name || product.category?.name || product.category || "-"}</Card.Text>
                        <div className="d-flex justify-content-between align-items-center">
                          <span className="h5 mb-0">₹{price}</span>
                          <Button variant="outline-primary" size="sm" onClick={e => handleAddToCart(product, e)}>Add to Cart</Button>
                        </div>
                      </Card.Body>
                    </Card>
                  </Col>
                  ...
                  <Card className="h-100 shadow-sm hover-shadow">
                    ...
                    <Card.Body>
                      ...
                      <div className="d-flex justify-content-between align-items-center">
                        ...
                        <Button variant="outline-primary" size="sm" disabled>Add to Cart</Button>
                      </div>
                    </Card.Body>
                  </Card>
                </Col>
          ...
          <Row xs={1} md={2} lg={4} className="g-4 justify-content-center">
            {brandProducts.length > 0 && brandProducts.some(bp => bp.product) ? (
              brandProducts.map((bp, i) => {
                ...
                const price = (variant && variant.price) || (product.variantDetails && product.variantDetails.length > 0 && product.variantDetails[0].price) || product.price || "--";
                return (
                  <Col key={product._id || i} className="d-flex justify-content-center">
                    <Card className="h-100 shadow-sm" style={{ width: "100%", maxWidth: 250, position: 'relative' }}>
                      <div className="position-relative">
                        <Card.Img
                          variant="top"
                          src={image}
                        ...
                        {(() => {
                          const variantId = variant?._id || variant?.id;
                          const productId = product._id || product.id;
                          const isWishlisted = variantId ? wishlist.some(item => item.id === productId && item.variant === variantId) : false;
                          return (
                            <Button
                              variant="light"
                              size="sm"
                      ...
                      <Card.Body className="text-center">
                        ...
                        <Card.Text className="text-muted small">{brand}</Card.Text>
                        <div className="d-flex justify-content-between align-items-center">
                          <span className="h5 mb-0">₹{price}</span>
                          <Button variant="outline-primary" size="sm" onClick={e => handleAddToCart(product, e)}>Add to Cart</Button>
                        </div>
              ...
              [1, 2, 3, 4].map((i) => (
                <Col key={`brand-placeholder-${i}`} className="d-flex justify-content-center">
                  <Card className="h-100 shadow-sm" style={{ width: "100%", maxWidth: 250 }}>
                    <Card.Img
                      variant="top"
                      src={`https://via.placeholder.com/300x300?text=No+Image`}
                      alt={`Placeholder Product ${i}`}
      ...
      <Container className="py-5">
        ...
        {loadingNewLaunches ? (
          ...
          <Row xs={2} sm={2} md={3} lg={3} className="g-4 justify-content-center">
            {newLaunches.length > 0 ? (
              newLaunches.map((product, i) => {
                ...
                return (
                  <Col key={product._id || i} className="d-flex justify-content-center">
                    <Card className="h-100 shadow-sm" style={{ width: "100%", maxWidth: 220, position: 'relative' }}>
                      <div className="position-relative">
                        <Card.Img
                          variant="top"
                          src={image}
                      ...
                      <Card.Body className="text-center">
                        <Card.Title>{product.name}</Card.Title>
                        <Card.Text className="text-muted small">Just Launched</Card.Text>
                        <div className="d-flex justify-content-between align-items-center">
                          <span className="h5 mb-0">₹{price}</span>
                          <Button variant="outline-primary" size="sm" onClick={e => handleAddToCart(product, e)}>Add to Cart</Button>
                        </div>
              ...
              [1, 2, 3, 4, 5, 6].map((i) => (
                <Col key={`new-launch-placeholder-${i}`} className="d-flex justify-content-center">
                  <Card className="h-100 shadow-sm" style={{ width: "100%", maxWidth: 220 }}>
                    <Card.Img
                      variant="top"
                      src={`https://via.placeholder.com/300x300?text=No+Image`}
        ...
        {loadingHandPicked ? (
          ...
          <Row xs={1} md={2} lg={4} className="g-4 justify-content-center">
            {handPicked.length > 0 ? (
                ...
                return (
                  <Col key={product._id || i} className="d-flex justify-content-center">
                    <Card className="h-100 shadow-sm" style={{ width: "100%", maxWidth: 250, position: 'relative' }}>
                      ...
                      <Card.Body className="text-center">
                        <Card.Title>{product.name}</Card.Title>
                        <Card.Text className="text-muted small">Colour: {colour}</Card.Text>
                        <div className="d-flex justify-content-between align-items-center">
                          <span className="h5 mb-0">₹{price}</span>
                          <Button variant="outline-primary" size="sm" onClick={e => handleAddToCart(product, e)}>Add to Cart</Button>
                        </div>
              ...
              [1, 2, 3, 4].map((i) => (
                <Col key={`hand-picked-placeholder-${i}`} className="d-flex justify-content-center">
                  <Card className="h-100 shadow-sm" style={{ width: "100%", maxWidth: 250 }}>
                    <Card.Img
                      variant="top"
                      src={`https://via.placeholder.com/300x300?text=No+Image`}
                    ...
                    <Card.Body className="text-center">
                      ...
                      <Card.Text className="text-muted small">Colour: -</Card.Text>
                      <div className="d-flex justify-content-between align-items-center">
                        <span className="h5 mb-0">₹--</span>
                      </div>
                    </Card.Body>
